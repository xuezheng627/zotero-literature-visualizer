...
- a publication timeline grouped by year, with theme-colored, clickable paper nodes;
...
- The publication timeline is generated automatically from `publication_date`
  (or another available year field) and `paper_assignments[*].theme`; no
  separate timeline data needs to be authored in the spec. Keep publication
  dates normalized so timeline ordering is reliable.
