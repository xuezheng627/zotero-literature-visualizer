...
- For Chinese-facing dashboards, write concise Chinese research summaries for
  the research topic, method, and findings. Do not place untranslated English
  evidence under a Chinese label; keep the original English as a collapsed
...
- The dashboard should include paper cards, a publication timeline grouped by
  year with theme-colored, clickable paper nodes, bilingual detail notes, theme
  donut, method donut, theme-method flow map, official journal homepage links,
  DOI links, and local PDF launcher links when files exist.
