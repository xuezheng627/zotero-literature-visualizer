...
def load_spec(path: Path, papers: list[dict[str, Any]], title: str, subtitle: str) -> dict[str, Any]:
    ...
    return {
        ...
        "paper_assignments": assignments,
        "details": details,
        "curated_chinese": spec.get("curated_chinese") if isinstance(spec.get("curated_chinese"), dict) else {},
        "curated_chinese_extended": spec.get("curated_chinese_extended") if isinstance(spec.get("curated_chinese_extended"), dict) else {},
    }
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .timeline-legend { display:flex; flex-wrap:wrap; gap:8px 12px; margin:12px 0 0; color:var(--muted); font-size:12px; }
    .timeline-legend span { display:inline-flex; align-items:center; gap:6px; }
    .timeline-legend i { width:9px; height:9px; border-radius:999px; display:inline-block; }
    .timeline-scroll { overflow:hidden; padding:24px 2px 8px; }
    .timeline { display:grid; gap:9px; min-width:0; align-items:start; }
    .timeline-year { position:relative; min-width:0; border-top:2px solid var(--line); padding-top:15px; min-height:110px; }
    .timeline-year::before { content:""; position:absolute; top:-7px; left:0; width:12px; height:12px; border-radius:999px; background:var(--ink); }
    .timeline-year h3 { font-size:clamp(11px, 1.45vw, 17px); white-space:nowrap; }
    .timeline-year p { color:var(--muted); font-size:12px; margin:4px 0 10px; }
    .timeline-papers { display:grid; grid-template-columns:repeat(auto-fit, minmax(14px, 1fr)); gap:6px; }
    .timeline-item { width:14px; height:14px; min-height:14px; padding:0; justify-self:center; border:2px solid #fff; border-radius:999px; box-shadow:0 0 0 1px rgba(24,33,41,.16); }
    .timeline-item:hover, .timeline-item:focus-visible { transform:scale(1.35); box-shadow:0 0 0 2px rgba(24,33,41,.3); outline:none; }
    ...
    .original-note { margin-top:8px; color:var(--muted); font-size:12px; }
    .original-note summary { cursor:pointer; color:#285d7d; }
    .original-note p { color:var(--muted); font-size:12px; }
...
<main>
  ...
  <section class="panel" style="margin-top:14px;">
    <h2>发表时间线 / Publication Timeline</h2>
    <p class="hint">按发表年份浏览文献；每张节点卡的边线和主题标签对应其主主题。点击节点可打开文章详情。</p>
...
<script>
  ...
  const curatedChinese = spec.curated_chinese || {};
  const curatedChineseExtended = spec.curated_chinese_extended || {};
  const radius = 67;
  ...
  function detailText(value, sectionKey) {
    const fallback = {
      data: "本文的样本、案例、参数及软件设置应以 PDF 原文为准；用于比较或引用前，请核对研究对象和边界。",
      limits: "本卡用于文献导航与初步比较；研究结论、适用条件和局限应以原文的研究设计与结果为准。",
      relevance: "该文从主题、方法或应用情境上补充了本库关于租赁装配式建筑的证据。"
    };
    if (value && typeof value === "object") {
      const curated = curatedChineseExtended[String(activeRank)]?.[sectionKey] || curatedChinese[String(activeRank)]?.[sectionKey];
      const chinese = curated || (sectionKey === "data" ? fallback.data : sectionKey === "limits" ? fallback.limits : sectionKey === "relevance" ? fallback.relevance : value.zh);
      const english = value.en;
      const original = english ? `<details class="original-note"><summary>英文原文参考 / English evidence</summary><p>${escapeHtml(english)}</p></details>` : "";
      return `<p><span class="lang-label">中</span>${escapeHtml(chinese)}</p>${original}`;
    }
    return `<p>${escapeHtml(value || "暂无摘要。")}</p>`;
  }
  function detailSection(sectionKey, title, value) {
    return `<section class="detail-section"><h3>${title}</h3>${detailText(value, sectionKey)}</section>`;
  }
  ...
  function publicationYear(paper) {
    const match = String(paper.publication_date || "").match(/\b(?:19|20)\d{2}\b/);
  ...
  function renderTimeline() {
    ...
    document.getElementById("timelineLegend").innerHTML = themeDefs.map(def =>
      `<span><i style="background:${def.color}"></i>${escapeHtml(def.name)}</span>`
    ).join("");
    document.getElementById("timeline").innerHTML = years.map(year => {
      const rows = byYear.get(year).slice().sort((a, b) => String(a.title).localeCompare(String(b.title)));
      const cards = rows.map(paper => {
        const theme = paperTheme(paper);
        const color = colorFor(themeDefs, theme);
        const label = `#${paper.rank} ${paper.title} — ${theme}`;
        return `<button class="timeline-item" type="button" data-rank="${paper.rank}" style="background:${color}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}"></button>`;
      }).join("");
      return `<article class="timeline-year"><h3>${escapeHtml(year)}</h3><p>${rows.length} paper${rows.length === 1 ? "" : "s"}</p><div class="timeline-papers">${cards}</div></article>`;
    }).join("");
    document.getElementById("timeline").style.gridTemplateColumns = `repeat(${Math.max(years.length, 1)}, minmax(0, 1fr))`;
    document.querySelectorAll(".timeline-item").forEach(button => button.addEventListener("click", () => openDetail(button.dataset.rank)));
  }
  ...
  function openDetail(rank) {
    ...
    document.getElementById("detailSections").innerHTML = [
      detailSection("topic", "研究主题 / Research Theme", detail.topic),
      detailSection("method", "方法 / Method", detail.method),
      detailSection("data", "数据或案例 / Data or Case", detail.data),
      detailSection("findings", "主要结果 / Findings", detail.findings),
      detailSection("limits", "局限 / Limitations", detail.limits),
      detailSection("relevance", "为什么重要 / Relevance", detail.relevance)
    ].join("");
...
def build_dashboard(args: argparse.Namespace) -> None:
    ...
    data_payload = {"config": payload.get("config", {}), "papers": papers, "spec": {key: spec[key] for key in ("title", "subtitle", "layout", "theme_definitions", "subtheme_definitions", "method_definitions", "paper_assignments", "curated_chinese", "curated_chinese_extended")}}
