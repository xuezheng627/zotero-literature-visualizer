...
<main>
  ...
  <nav class="tabs" aria-label="Dashboard views">
    ...
    <button class="tab" type="button" data-tab="timeline">Timeline / 时间线</button>
  ...
  <section class="view" id="view-timeline">
    <article class="panel">
      <div class="panel-head"><div><h2>发表时间线 / Publication Timeline</h2><p class="hint">按发表年份排列；节点边线与标签呈现文章的主主题。点击节点打开文章详情。</p></div></div>
...
function renderTimeline() {
  ...
  document.getElementById("timeline").innerHTML = years.map(year => {
    ...
    const cards = rows.map(paper => {
      ...
      return `<button class="timeline-item" type="button" data-rank="${paper.rank}" style="background:${colorForTheme(theme)}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}"></button>`;
    }).join("");
...
function detailText(value, sectionKey, rank) {
  const fallback = {
    data: "本文的样本、案例、参数及软件设置应以 PDF 原文为准；用于比较或引用前，请核对研究对象和边界。",
    limits: "本卡用于文献导航与初步比较；研究结论、适用条件和局限应以原文的研究设计与结果为准。",
    relevance: "该文从主题、方法或应用情境上补充了本库的证据。"
  };
  if (value && typeof value === "object") {
    const curated = curatedChineseExtended[String(rank)]?.[sectionKey] || curatedChinese[String(rank)]?.[sectionKey];
    ...
    return `<p><span class="lang-label">中</span>${escapeHtml(chinese || "暂无摘要。")}</p>${original}`;
  }
  return `<p>${escapeHtml(value || "No note yet.")}</p>`;
}
function detailSection(sectionKey, title, value, rank) {
  return `<section class="detail-section"><h3>${title}</h3>${detailText(value, sectionKey, rank)}</section>`;
}
function openDetail(rank) {
  ...
  document.getElementById("detailSections").innerHTML = [
    detailSection("topic", "研究主题 / Research Theme", detail.topic, rank),
    detailSection("method", "方法 / Method", detail.method, rank),
    detailSection("data", "数据或案例 / Data or Case", detail.data, rank),
    detailSection("findings", "主要结果 / Findings", detail.findings, rank),
    detailSection("limits", "局限 / Limitations", detail.limits, rank),
    detailSection("relevance", "为什么重要 / Relevance", detail.relevance, rank)
  ].join("");
